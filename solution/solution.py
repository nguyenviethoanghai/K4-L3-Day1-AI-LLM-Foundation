...
def run_assistant(
    ...
    while True:
        ...
        num_turns += 1
        total_tokens += count_tokens(user_msg, model=OPENAI_MODEL) + count_tokens(reply, model=OPENAI_MODEL)
        total_cost += estimate_cost(user_msg, reply, model=OPENAI_MODEL)["total_cost"]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for item in results:
        prompt_str = truncate(str(item.get("prompt", "")))
        gpt4o_res = truncate(str(item.get("gpt4o_response", "")))
        mini_res = truncate(str(item.get("mini_response", "")))
        gpt4o_lat = f"{item.get('gpt4o_latency', 0.0):.2f}s"
        mini_lat = f"{item.get('mini_latency', 0.0):.2f}s"

        row = f"{prompt_str:<40} | {gpt4o_res:<40} | {mini_res:<40} | {gpt4o_lat:<14} | {mini_lat:<12}"
        lines.append(row)
